...
public class Leetcode {
	 ...
	 * @param candidates
	 * @param target
	 * @return    2,3,6,7---7       2,3,5----8   
	 * 先假定数组已经排序过，最大数不大于target,集合没有重复数字
	 */
    public List<List<Integer>> combinationSum2(int[] candidates, int target) {
    	List<List<Integer>> Result=new ArrayList<>();
        int max=target/candidates[0];//每个结果集最大长度
        
        Result=combinationSum_Help(candidates,target,0,Result);
     ...
     * @return
     */
    private List<List<Integer>> combinationSum_Help(int[] candidates, int target,int i,List<List<Integer>> Result)
    {
    	int sum=0;
    	List<Integer> tempal=new ArrayList<>();
    	while (sum!=target) {
			if (candidates[i]<target) {
				tempal.add(candidates[i]);
			}
			else if (i==target) {
    		...
    		Result=combinationSum_Help(candidates,target,i,Result);
	 ...
	 * 先假定数组已经排序过，最大数不大于target,集合没有重复数字
	 */
    public List<List<Integer>> combinationSum1Wrong(int[] candidates, int target) {
    	List<List<Integer>> Result=new ArrayList<>();
        int max=target/candidates[0];//每个结果集最大长度

        int a=0,b=candidates.length-1;
        //System.out.println(candidates[b]);
       // for (int i = 0; i < max; i++) {
        	for (int j = 0; j < candidates.length; j++) {
        		...
        		if (candidates[a]+candidates[b]==target) {
    				...
    				tempal.add(candidates[a]);
    				tempal.add(candidates[b]);
    				Result.add(tempal);
    				//System.out.println("aaa");
    				a++;
    				b--;
    			}
        		else if (candidates[a]+candidates[b]>target) {
					b--;
				}
        		else {
					a++;
    ...
    public void nextPermutation(int[] nums) {
    	...
    	Arrays.sort(nums);
    	//System.out.println("aaa=="+Arrays.toString(nums));
    }
     ...
     * @param nums
     */
    public void nextPermutation1Wrong(int[] nums) {
        ...
        for (int i = nums.length-1; i>0; i--) {
			...
			if (nums[i]>nums[i-1]) {
				...
				if (i==1&&nums.length!=2) {//如果是最高位
					...
					if (!flagB) {
						//System.out.println("如果循环到头也没有比nums小的");
						int temp=nums[0];
	 ...
	 * @return
	 */
    public List<List<Integer>> threeSumTimeOut(int[] nums) {
    	log.debug(System.currentTimeMillis());
    	//Arrays.sort(nums);
    	List<List<Integer>> result=new ArrayList<>();
				...
				for (int k = j+1; k < nums.length; k++) {
					...
					if (nums[i]+nums[j]+nums[k]==0) {
						...
						for (int l = 0; l < result.size(); l++) {
							 ...
							 * 测试和包含全是0,0,0的list比较
							Integer[] aaa={0,0,0};
							if (result.get(l).containsAll(Arrays.asList(aaa))) {
								System.out.println("bbbbbbbbbbbbbbbb");
							}
							if (Arrays.asList(aaa).containsAll(result.get(l))) {
								System.out.println("cccccccccccccccc");
							}
							*/
								
							if (result.get(l).containsAll(temp)&&temp.containsAll(result.get(l))) {
								flag=true;
								break;
							}
						}
						if (!flag) {
							result.add(temp);
							//System.out.println("i=="+i+"_"+nums[i]+"j=="+j+"_"+nums[j]+"k=="+k+"_"+nums[k]);
						}
					}
				}
			}
